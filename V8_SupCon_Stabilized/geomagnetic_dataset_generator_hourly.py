"""
Geomagnetic Dataset Generator - Hourly Version
Generate spectrogram dataset dengan proses hanya 1 jam data sesuai waktu kejadian gempa
Menggunakan data lokal dari folder mdata/
"""
...
class GeomagneticDatasetGeneratorHourly:
    """Generate dataset spectrogram dari data geomagnetic (1 jam data)"""
    
    def __init__(self, server_path='mdata', output_dir='dataset_spectrogram_hourly'):
        """
        Initialize generator
        
        Args:
            server_path: Path ke folder data lokal
        ...
        logger.info(f"GeomagneticDatasetGeneratorHourly initialized")
    ...
    def fetch_hour_data(self, year, month, day, hour, station):
        """
        Fetch data 1 jam dari file lokal
        ...
        try:
            # Read full day data
            data = read_604rcsv_new_python(year, month, day, station, self.server_path)
            ...
            if end_idx > len(data['H']):
                ...
                end_idx = min(start_idx + 3600, len(data['H']))
            
            if start_idx >= len(data['H']):
                logger.error(f"Start index {start_idx} exceeds data length {len(data['H'])}")
                return None
            ...
            hour_data = {
                ...
                'Time': np.arange(len(data['H'][start_idx:end_idx])) / 3600.0
            }
    ...
    def _save_reports(self, success_list, failure_list):
        ...
        with pd.ExcelWriter(report_path, engine='openpyxl') as writer:
            # Summary
            summary_data = {
                'Metric': ['Total Events', 'Successfully Processed', 'Failed', 'Success Rate (%)'],
                'Value': [
...
def main():
    ...
    parser.add_argument('--output-dir', default='dataset_spectrogram_hourly',
    ...
    # Create generator
    generator = GeomagneticDatasetGeneratorHourly(
